defmodule Moola.GDAXQuery do
  ...
  def ticker_query(options), do: (from item in Ticker) |> ticker_query(options)
  ...
  def latency_query(options), do: (from item in OrderLatency) |> latency_query(options)

  def latency_query(query, options) do
  ...
  def query_latency(options), do: options |> latency_query |> latency_order(options) |> limit_query(options) |> Repo.all

  def latency_order(query, options) do
    from item in query, order_by: [desc: item.timestamp]
  end
